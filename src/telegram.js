...
function mainKeyboard() {
  ...
  if (discordInvite()) kb.row().url('Apri Discord', discordInvite());
  return kb;
}
...
async function createInviteLinkSafe() {
  ...
  try {
    ...
    return await bot.api.createChatInviteLink(channelId(), { expire_date: expire, member_limit: 1, name: 'VIP automatico' });
  } catch (error) {
...
export function setupTelegramHandlers() {
  bot.command('start', async (ctx) => {
    if (ctx.from) upsertTelegramUser(ctx.from.id, ctx.from.username || '');
  ...
  bot.callbackQuery('gen_code', async (ctx) => {
    ...
    ].join('\n'), { reply_markup: discordInvite() ? new InlineKeyboard().url('Apri Discord', discordInvite()) : undefined });
  });
  ...
  bot.on('message:successful_payment', async (ctx) => {
    ...
    const expires = activateVip(ctx.from.id, ctx.from.username || '', durationDays(), payment);
    const row = getSubByTelegram(ctx.from.id);
    ...
    if (row?.discord_id) await setVipRole(row.discord_id, true, expires);
    ...
    await ctx.reply([
      ...
      '',
      row?.discord_id ? 'Il ruolo VIP Discord è stato assegnato.' : 'Ora genera il codice e collegalo su Discord per ricevere il ruolo VIP.',
